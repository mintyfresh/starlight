...
RSpec.describe Event::RemovePlayerFromRoleSubscriber, type: :subscriber do
  subject(:subscriber) { described_class }

  it 'accepts Registration::DestroyMessage when the event has a Discord role' do
    event = create(:event, :published)
    create(:event_role_config, :with_discord_role_id, event:)
    registration = create(:registration, event:)
    expect(subscriber).to accept(Registration::DestroyMessage.new(registration:))
  end

  it 'does not accept Registration::DestroyMessage when the event does not have a Discord role' do
    event = create(:event, :published)
    registration = create(:registration, event:)
    expect(subscriber).not_to accept(Registration::DestroyMessage.new(registration:))
  end
  ...
  describe '#perform' do
    ...
    let(:message) { Registration::DestroyMessage.new(registration:) }
    ...
    it "adds the player to the event's Discord role" do
      ...
      add_guild_member_role = stub_discord_remove_guild_member_role(
